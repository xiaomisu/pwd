...
export const handleConfigureAuth = (config: Record<string, any>) => {
...
export const handleNetworkError = (errStatus?: number): void => {
  ...
  message.open({ type: "error", content: "无法连接到服务器！" });
};
...
export const handleAuthError = (errno: string): boolean => {
  ...
  if (Object.prototype.hasOwnProperty.call(authErrMap, errno)) {
    message.open({ type: "error", content: authErrMap[errno] });
    // 授权错误，登出账户
    // logout();
    return false;
  }
...
export const handleGeneralError = (errno: string, errmsg: string): boolean => {
  if (errno !== "0") {
    message.open({ type: "error", content: errmsg });
    return false;
  }
